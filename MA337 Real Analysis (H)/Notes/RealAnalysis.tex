...
\begin{example}
    ...
    Let $S = \{ [a,b) : a,b \in \mathbb{Q},\, a < b \}$, and define the pre-measure $\mu([a,b)) = b - a$. The outer measure $\mu^*$ on $2^{\mathbb{R}}$ is defined by $\mu^*(E) = \inf \{ \sum_{j=1}^{\infty} \mu(A_j) : A_j \in S,\, E \subseteq \bigcup_{j=1}^{\infty} A_j \}$.
...
\underline{Recall}: A triple ($\Omega, \mathcal{A}, \mu$) ($\mathcal{A}$ is some $\sigma$-algebra on the set $\Omega$) is called a \textbf{measure space (spcae with measure)}.
