...
\section{Crash Course on Set Theory}
...
$A \in \mathcal{M}(\Omega)$ if $\forall \eps > 0$, $\exists B_{\eps} \in \mathcal{A}(S)$ such that $\mu^*(A \triangle B_{\eps}) < \eps.$
...
So, we end up with a triple: ($\Omega, \mathcal{M}(\Omega), \mu$) --- (set $\Omega$, $\sigma$-algebra, $\sigma$-additive measure on $\mathcal{M}(\Omega)$).

\begin{definition}
    (Space with Measure)

    Such a triple ($\Omega, \mathcal{A}, \mu$) ($\mathcal{A}$ is some $\sigma$-algebra) is called a \textbf{spcae with measure}.
\end{definition}
...
\begin{definition}
    ...
    If ($\Omega, \mathcal{A}, \mu$) is a space with measure, then it's called \textbf{complete} if $
    \forall A \in \mathcal{A}$ with $\mu(A)=0$, it holds $\forall E \subset A, E \in \mathcal{A}$ (and hence $\mu(E)=0$).
\end{definition}

\begin{example}
    For ($\Omega, \mathcal{M}(\Omega), \mu$), we always have completeness:

    $\mu(A) = 0, E \subset A \Rightarrow 0 \leq \mu^*(E) \leq \mu^*(A) = 0 \Rightarrow E \subset \mathcal{M}(\Omega)$.
...
\begin{theorem}
    For any space with measure ($\Omega, \mathcal{A}, \mu$), the following holds:
    \begin{enumerate}
        \item $\forall A_1 \subset A_2 \subset A_3 \subset \ldots$ with $A_i \in \mathcal{A}$, it holds $\mu(\bigcup_{i=1}^{\infty} A_i) = limit i \rightarrow \infty \mu(A_i)$.
        \item 
    \end{enumerate}

\end{theorem}

Question: What about $\infty$-valued measures?

Consider a space with measure ($\Omega, \mathcal{A}, \mu$), where $\mu$ is a $\bar{\R}$-valued measure. The definition of $\sigma$-additivity is repeated word-by-word.
...
\begin{proposition}
    \begin{enumerate}
        ...
        \item If $A \subset \bigcup_{j=1}^{\infty} A_j,$ then $\mu(\bigcup_{j=1}^n A_j) = \Sigma_{j=1}^{\infty} \mu(A_j)$.
    \end{enumerate}
\end{proposition}

\begin{definition}
    A space with $\infty$-valued measure is called \textbf{$\sigma$-finite} if $\Omega = \bigsqcup_{k=1}^{\infty} \Omega_k, \Omega_k \in \mathcal{A}, \mu(\Omega_k) < \infty$, then $\forall A \subset \mathcal{A}, \mu(A) = \Sigma_{k=1}^{\infty} \mu(A \bigcap \Omega_k) =: \Sigma_{k=1}^{\infty} \mu_k(A)$.
\end{definition}
